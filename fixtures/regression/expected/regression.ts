...
const _rt = await UniffiRuntime.load(new URL('./regression.wasm', import.meta.url), 'crate_name');
...
_rt.registerCallbackVTable('Transformer', 'uniffi_crate_name_fn_init_callback_vtable_transformer', [
  {
    params: ['i64'], results: [],
...
export class SafeDivider {
  ...
  private constructor(handle: bigint) {
    this._handle = handle;
    _rt.registerPointer(this, 'uniffi_crate_name_fn_free_safedivider', handle);
  }
  /** @internal */
  ...
  static create(initial: number): SafeDivider {
    ...
    try {
      _rt.call('uniffi_ffibuffer_crate_name_fn_constructor_safedivider_new', _argPtr, _retPtr);
  ...
  static fromPositive(initial: number): SafeDivider {
    ...
    const _retPtr = _rt.scratchAlloc(5 * 8);
    try {
      _rt.call('uniffi_ffibuffer_crate_name_fn_constructor_safedivider_from_positive', _argPtr, _retPtr);
  ...
  divide(divisor: number): number {
    this._assertLive();
    const _clonedHandle = _rt.cloneObjectHandle('uniffi_crate_name_fn_clone_safedivider', this._handle);
    const _argPtr = _rt.scratchAlloc(2 * 8);
    _rt.writeHandleElement(_argPtr, _clonedHandle);
    _rt.writeI32Element(_argPtr + 8, divisor);
    const _retPtr = _rt.scratchAlloc(5 * 8);
    try {
      _rt.call('uniffi_ffibuffer_crate_name_fn_method_safedivider_divide', _argPtr, _retPtr);
      _rt.checkCallStatus(_retPtr + 8, (rb) => _liftErrorDivError(rb));
  ...
  async squareAsync(): Promise<number> {
    ...
    _rt.call('uniffi_ffibuffer_crate_name_fn_method_safedivider_square_async', _argPtr, _retPtr);
    const _futureHandle = _rt.readHandleElement(_retPtr);
    try {
      await _rt.pollToReady(_futureHandle, 'ffi_crate_name_rust_future_poll_i32');
      _rt.scratchReset();
      const _statusPtr = _rt.scratchAlloc(32);
      _rt._writeRustCallStatusStruct(_statusPtr);
      const _result = (_rt.getExport('ffi_crate_name_rust_future_complete_i32') as any)(_futureHandle, _statusPtr);
      _rt.checkCallStatus(_statusPtr);
      return _result;
    } finally {
      _rt.scratchReset();
      (_rt.getExport('ffi_crate_name_rust_future_free_i32') as any)(_futureHandle);
    }
  }
  ...
  free(): void {
    ...
    _rt.callFree('uniffi_crate_name_fn_free_safedivider', this._handle);
  }
}
...
export namespace Regression {
  ...
  export function apply(t: Transformer, value: string): string {
    ...
    try {
      _rt.call('uniffi_ffibuffer_crate_name_fn_func_apply', _argPtr, _retPtr);
