...
export namespace SimpleFns {
  ...
  export async function greetAsync(name: string): Promise<string> {
    ...
    _rt.call('uniffi_ffibuffer_simple_fns_fn_func_greet_async', _argPtr, _retPtr);
    const _futureHandle = _rt.readHandleElement(_retPtr);
    try {
      await _rt.pollToReady(_futureHandle, 'ffi_simple_fns_rust_future_poll_rust_buffer');
      _rt.scratchReset();
