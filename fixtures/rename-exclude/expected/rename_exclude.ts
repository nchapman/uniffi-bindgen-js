...
const _rt = await UniffiRuntime.load(new URL('./rename_exclude.wasm', import.meta.url), 'crate_name');
...
export namespace RenameExclude {
  export function greet(name: string): string {
    ...
    try {
      _rt.call('uniffi_ffibuffer_crate_name_fn_func_hello', _argPtr, _retPtr);
  ...
  export function version(): string {
    ...
    const _retPtr = _rt.scratchAlloc(7 * 8);
    try {
      _rt.call('uniffi_ffibuffer_crate_name_fn_func_version', _argPtr, _retPtr);
      _rt.checkCallStatus(_retPtr + 24);
      const _result = _rt.liftString(_rt.readRustBufferElements(_retPtr));
      return _result;
    } finally {
