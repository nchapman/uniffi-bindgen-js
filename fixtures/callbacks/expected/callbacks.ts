...
const _rt = await UniffiRuntime.load(new URL('./callbacks.wasm', import.meta.url), 'crate_name');
...
_rt.registerCallbackVTable('Formatter', 'uniffi_crate_name_fn_init_callback_vtable_formatter', [
  {
    params: ['i64'], results: [],
...
export namespace Callbacks {
  export function applyFormatter(fmt: Formatter, template: string): string {
    ...
    try {
      _rt.call('uniffi_ffibuffer_crate_name_fn_func_apply_formatter', _argPtr, _retPtr);
  ...
  export function formatGreeting(fmt: Formatter, name: string): string {
    ...
    const _retPtr = _rt.scratchAlloc(7 * 8);
    try {
      _rt.call('uniffi_ffibuffer_crate_name_fn_func_format_greeting', _argPtr, _retPtr);
      _rt.checkCallStatus(_retPtr + 24);
      const _result = _rt.liftString(_rt.readRustBufferElements(_retPtr));
      return _result;
    } finally {
