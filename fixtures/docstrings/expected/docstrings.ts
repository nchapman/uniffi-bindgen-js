...
const _rt = await UniffiRuntime.load(new URL('./docstrings.wasm', import.meta.url), 'crate_name');
...
_rt.registerCallbackVTable('Processor', 'uniffi_crate_name_fn_init_callback_vtable_processor', [
  {
    params: ['i64'], results: [],
...
export class Counter {
  ...
  private constructor(handle: bigint) {
    this._handle = handle;
    _rt.registerPointer(this, 'uniffi_crate_name_fn_free_counter', handle);
  }
  /** @internal */
  ...
  static create(start: bigint): Counter {
    ...
    try {
      _rt.call('uniffi_ffibuffer_crate_name_fn_constructor_counter_new', _argPtr, _retPtr);
  ...
  get(): bigint {
    this._assertLive();
    const _clonedHandle = _rt.cloneObjectHandle('uniffi_crate_name_fn_clone_counter', this._handle);
    const _argPtr = _rt.scratchAlloc(1 * 8);
    _rt.writeHandleElement(_argPtr, _clonedHandle);
    const _retPtr = _rt.scratchAlloc(5 * 8);
    try {
      _rt.call('uniffi_ffibuffer_crate_name_fn_method_counter_get', _argPtr, _retPtr);
      _rt.checkCallStatus(_retPtr + 8);
  ...
  increment(): void {
    ...
    try {
      _rt.call('uniffi_ffibuffer_crate_name_fn_method_counter_increment', _argPtr, _retPtr);
  ...
  free(): void {
    ...
    _rt.callFree('uniffi_crate_name_fn_free_counter', this._handle);
  }
}
