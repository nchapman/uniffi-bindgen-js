...
const _rt = await UniffiRuntime.load(new URL('./custom_types.wasm', import.meta.url), 'crate_name');
...
export namespace CustomTypes {
  export function handlesEqual(a: Handle, b: Handle): boolean {
    ...
    try {
      _rt.call('uniffi_ffibuffer_crate_name_fn_func_handles_equal', _argPtr, _retPtr);
  ...
  export function makeHandle(seed: bigint): Handle {
    ...
    const _retPtr = _rt.scratchAlloc(5 * 8);
    try {
      _rt.call('uniffi_ffibuffer_crate_name_fn_func_make_handle', _argPtr, _retPtr);
      _rt.checkCallStatus(_retPtr + 8);
  ...
  export function normalizeUrl(url: Url): Url {
    ...
    try {
      _rt.call('uniffi_ffibuffer_crate_name_fn_func_normalize_url', _argPtr, _retPtr);
