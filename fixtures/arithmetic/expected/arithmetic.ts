...
const _rt = await UniffiRuntime.load(new URL('./arithmetic.wasm', import.meta.url), 'crate_name');
...
export namespace Arithmetic {
  ...
  export function divide(a: number, b: number): number {
    ...
    try {
      _rt.call('uniffi_ffibuffer_crate_name_fn_func_divide', _argPtr, _retPtr);
  ...
  export function sqrt(x: number): number {
    ...
    const _retPtr = _rt.scratchAlloc(5 * 8);
    try {
      _rt.call('uniffi_ffibuffer_crate_name_fn_func_sqrt', _argPtr, _retPtr);
      _rt.checkCallStatus(_retPtr + 8, (rb) => _liftErrorMathError(rb));
      const _result = _rt.readF64Element(_retPtr);
      return _result;
    } finally {
