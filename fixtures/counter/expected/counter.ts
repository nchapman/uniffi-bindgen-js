...
const _rt = await UniffiRuntime.load(new URL('./counter.wasm', import.meta.url), 'crate_name');
...
export class Counter {
  ...
  decrement(): void {
    ...
    try {
      _rt.call('uniffi_ffibuffer_crate_name_fn_method_counter_decrement', _argPtr, _retPtr);
  ...
  resetTo(value: bigint): void {
    ...
    const _retPtr = _rt.scratchAlloc(4 * 8);
    try {
      _rt.call('uniffi_ffibuffer_crate_name_fn_method_counter_reset_to', _argPtr, _retPtr);
      _rt.checkCallStatus(_retPtr);
    } finally {
      _rt.scratchReset();
    }
